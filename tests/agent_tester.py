...
SCENARIOS: List[Scenario] = [
    # ── Issues: errores técnicos genéricos ───────────────────
    Scenario(
        name="Database connection error",
        query="connection refused database retry",
        expected_keywords=["connection", "refused", "retry"],
        expected_table="issues",
        description="Agente buscando cómo resolver error de conexión a base de datos",
    ),
    Scenario(
    ...
    Scenario(
        name="Race condition balance negativo",
        query="balance negative constraint concurrent race condition",
        expected_keywords=["balance", "negative", "constraint"],
        expected_table="issues",
        description="Agente buscando error de balance negativo por race condition",
    ),
    Scenario(
        name="RLS policy violation",
        query="RLS policy violation access control",
        expected_keywords=["rls", "policy", "violation"],
        expected_table="issues",
        description="Agente buscando error de políticas de control de acceso",
    ),
    Scenario(
        name="FTS5 special chars crash",
        query="FTS5 MATCH OperationalError special characters sanitize",
    ...
    Scenario(
        ...
        query="API call timeout circuit breaker",
        expected_keywords=["timeout", "api", "circuit", "breaker"],
        expected_table="issues",
        description="Agente buscando solución para timeouts en llamadas a API",
    ),

    # ── Docs: reglas de VELMA ────────────────────────────────
    Scenario(
        name="Constraint archivar solo humano",
        query="archivar archived solo humano agente nunca",
        expected_keywords=["archivar", "humano", "agente", "nunca"],
        expected_table="docs",
        expected_chunk_type="constraint",
        description="Agente verificando quién puede archivar entradas del KB",
    ),
    Scenario(
        name="Constraint evidencia para success",
        query="evidencia obligatoria outcome success marcar",
        expected_keywords=["evidencia", "success", "outcome"],
        expected_table="docs",
        expected_chunk_type="constraint",
        description="Agente verificando qué se necesita para marcar un issue como resuelto",
    ),
    Scenario(
        name="Procedimiento registrar issue",
        query="como registrar issue resuelto knowledge base attempts",
        expected_keywords=["issue", "registrar", "outcome", "success"],
        expected_table="docs",
        expected_chunk_type="procedure",
        description="Agente buscando cómo registrar correctamente un error resuelto",
    ),
    Scenario(
        name="Regla confianza minima",
        query="score confianza minimo 0.75 similitud resultado",
        expected_keywords=["0.75", "confianza", "similitud"],
        expected_table="docs",
        expected_chunk_type="rule",
        description="Agente verificando el umbral mínimo de confianza para usar el KB",
    ),
    Scenario(
        name="Regla citar fuente",
        query="citar fuente ID score similitud issue",
        expected_keywords=["citar", "ID", "score", "similitud"],
        expected_table="docs",
        expected_chunk_type="rule",
        description="Agente buscando cómo citar correctamente una entrada del KB",
    ),
    Scenario(
        name="Procedimiento session summary",
        query="session summary cerrar reasoning log",
        expected_keywords=["session", "summary", "reasoning"],
        expected_table="docs",
        expected_chunk_type="procedure",
        description="Agente buscando cómo cerrar una sesión correctamente",
    ),

    # ── Edge cases ───────────────────────────────────────────
    Scenario(
        name="Query con typo intencional",
        query="coneccion rehusada base datos",  # typos deliberados
        expected_keywords=["connection", "refused"],
        expected_table="any",
        description="FTS5 no tolera typos — este caso retorna vacio (comportamiento esperado)",
    ),
    Scenario(
        name="Query generico una palabra",
        query="error",
        expected_keywords=["error"],
        expected_table="any",
        description="Query de una sola palabra — debe retornar resultados",
    ),
    Scenario(
        name="Query cross-idioma",
        query="minimum balance constraint validation",
        expected_keywords=["balance", "constraint", "negative"],
        expected_table="issues",
        description="Query en ingles para issues en espanol/ingles mixto",
    ),
]
